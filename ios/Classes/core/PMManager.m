...
- (NSArray<PMAssetEntity *> *)getAssetEntityListWithGalleryId:(NSString *)id type:(int)type page:(NSUInteger)page pageCount:(NSUInteger)pageCount filterOption:(PMFilterOptionGroup *)filterOption {
    ...
    //NSLog(@"assetOptions: %@", assetOptions);
    ...
    NSMutableArray *reserveArr = [NSMutableArray array];
    [assetArray enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(PHAsset * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [reserveArr addObject:obj];
    }];

    assetArray = (PHFetchResult<PHAsset *> *)reserveArr;
    ...
    //NSLog(@"star: %d---end: %d", startIndex, endIndex);
